...
export async function ensureResumeParsed(supabase, resumeId) {
  ...
  try {
    ...
    const parsed = await parseResumeWithLlm(pdfText, { userId: resume.user_id, resumeId });
 ...
 * Runs through the AI credit gate (operation cost resolved from ai_operations).
 *
 * @param {{
 *   supabase: object,
 *   sessionId: string,
 ...
 *   operationSlug?: string,       // ai_operations slug (resolved from the action)
 *   idempotencyKey?: string|null, // client-generated; prevents double-charging retries
 * }} ctx
 * @returns {Promise<{ analysis: string, suggestions: object[], questions: string[] }>}
 */
export async function generateSuggestions({ supabase, sessionId, userId, parsed, goal = null, jobContext = null, operationSlug = 'resume_optimization', idempotencyKey = null }) {
  ...
  let result;
  try {
    result = await runWithCredits({
      userId,
      operationSlug,
      idempotencyKey: idempotencyKey || `optimize:${sessionId}:${Date.now()}`,
      referenceId: sessionId,
      feature: 'resume_optimization',
      run: () => completeJson(prompt),
    });
  } catch (err) {
    // Credit/entitlement failures must surface to the API (402/403).
    if (err && err.name === 'CreditsError') throw err;
    console.error('generateSuggestions LLM failed', err?.message || err);
 ...
 * Runs through the AI credit gate as 'resume_analysis' (free by default;
 * usage is still recorded for unit-economics measurement).
 * @param {string} pdfText
 * @param {{ userId: string, resumeId: string }} ctx
 */
async function parseResumeWithLlm(pdfText, { userId, resumeId }) {
  ...
  try {
    ...
    if (err && err.name === 'CreditsError') throw err;
    console.error('parseResumeWithLlm LLM failed', err?.message || err);
    result = { data: null, model: 'none' };
  }
