...
export async function completeJson(prompt) {
  ...
  if (!apiKey) {
    return { data: null, model: 'none', version: 'none' };
  }
  ...
  try {
    ...
    return { data: null, model, usage: json.usage, version: 'none' };
  }

  return { data: parsed, model, usage: json.usage, version: 'assessment-v1' };
}
